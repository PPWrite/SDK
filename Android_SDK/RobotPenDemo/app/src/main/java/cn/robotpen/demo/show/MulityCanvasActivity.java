...
public class MulityCanvasActivity extends Activity implements WhiteBoardView.CanvasManageInterface {
    ...
    Button clearnScreen;
    @BindView(R.id.delScreen)
    Button delScreen;
    @BindView(R.id.gotoPro)
    Button gotoPro;
    @BindView(R.id.gotoNext)
    Button gotoNext;
    @BindView(R.id.activity_mulity_canvas)
    RelativeLayout activityMulityCanvas;
    ...
    @OnClick({R.id.clearnScreen,R.id.delScreen, R.id.gotoPro, R.id.gotoNext})
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.clearnScreen:
                ...
                break;
            case R.id.delScreen:
                whiteBoardView.delCurrPage();
                break;
            case R.id.gotoPro:
                whiteBoardView.frontPage();
                break;
            case R.id.gotoNext:
                whiteBoardView.nextPage();
                break;
        }
    }
    ...
    *扫描监听
    ...
    PenService.OnScanDeviceListener onScanDeviceListener = new PenService.OnScanDeviceListener() {
        ...
        public void find(DeviceEntity deviceObject) {
            DeviceEntity lastDevice = mPenManage.getLastDevice(MulityCanvasActivity.this);
        ...
        public void complete(HashMap<String, DeviceEntity> hashMap) {
            if (!mPenManage.getIsStartConnect()) {
                Toast.makeText(MulityCanvasActivity.this, "暂未发现设备", Toast.LENGTH_SHORT).show();
            }
        }
        ...
        public void status(int i) {
            switch (i) {
                case Keys.REQUEST_ENABLE_BT:
                    Toast.makeText(MulityCanvasActivity.this, "蓝牙未打开", Toast.LENGTH_SHORT).show();
                ...
                case Keys.BT_ENABLE_ERROR:
                    Toast.makeText(MulityCanvasActivity.this, "设备不支持BLE协议", Toast.LENGTH_SHORT).show();
                    break;
            }
    ...
    private PenService.OnConnectStateListener onConnectStateListener = new PenService.OnConnectStateListener() {
        ...
        public void stateChange(String arg0, ConnectState arg1) {
            if (arg1 == ConnectState.CONNECTED) {
                Toast.makeText(MulityCanvasActivity.this, "设备已连接且连接成功！", Toast.LENGTH_SHORT).show();
                ...
                Toast.makeText(MulityCanvasActivity.this, "设备已断开", Toast.LENGTH_SHORT).show();
            }
        }
    };
